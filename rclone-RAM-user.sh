...
need_cmds() {
    for cmd in rclone awk numfmt ps id mountpoint mount grep sed tr printf sudo mkdir chmod chown ss tail nemo mktemp rmdir curl jq tput du df find sleep seq; do
...
collect_inputs() {
    ...
    RAM_PERCENT=$(prompt_default "Percent of MemAvailable to allocate to RAM cache" "${DEFAULT_RAM_PERCENT}")
    ...
    MOUNTPOINT="${HOME}/${safe_remote}"
    CACHE_DIR="${HOME}/.rclone-cache-${safe_remote}"
}
...
prepare_layout() {
    ...
    CACHE_TMPFS_KB=$(( mem_available_kb * RAM_PERCENT / 100 ))
    local min_cache_kb=$(( MIN_CACHE_MIB * 1024 ))
    if [ "${CACHE_TMPFS_KB}" -lt "${min_cache_kb}" ]; then
        CACHE_TMPFS_KB="${min_cache_kb}"
    fi

    FREE_HEADROOM_KB=$(( CACHE_TMPFS_KB / 10 ))
    local min_free_kb=$(( MIN_FREE_MIB * 1024 ))
    if [ "${FREE_HEADROOM_KB}" -lt "${min_free_kb}" ]; then
        FREE_HEADROOM_KB="${min_free_kb}"
    fi

    if [ "${FREE_HEADROOM_KB}" -ge "${CACHE_TMPFS_KB}" ]; then
        FREE_HEADROOM_KB=$(( CACHE_TMPFS_KB / 4 ))
    fi

    CACHE_MAX_KB=$(( CACHE_TMPFS_KB - FREE_HEADROOM_KB ))
    if [ "${CACHE_MAX_KB}" -le 0 ]; then
        echo "Error: computed cache size is invalid." >&2
        exit 1
    fi

    CACHE_TMPFS_MIB=$(( CACHE_TMPFS_KB / 1024 ))
    CACHE_MAX_MIB=$(( CACHE_MAX_KB / 1024 ))
    FREE_HEADROOM_MIB=$(( FREE_HEADROOM_KB / 1024 ))

    local mem_available_human
    local cache_tmpfs_human
    local cache_max_human
    local free_human

    mem_available_human=$(numfmt --to=iec --suffix=B $(( mem_available_kb * 1024 )))
    cache_tmpfs_human=$(numfmt --to=iec --suffix=B $(( CACHE_TMPFS_KB * 1024 )))
    cache_max_human=$(numfmt --to=iec --suffix=B $(( CACHE_MAX_KB * 1024 )))
    free_human=$(numfmt --to=iec --suffix=B $(( FREE_HEADROOM_KB * 1024 )))

    echo "MemAvailable: ${mem_available_human}"
    echo "RAM cache tmpfs size: ${cache_tmpfs_human}"
    echo "Rclone VFS cache max size: ${cache_max_human}"
    echo "Reserved free space inside cache tmpfs: ${free_human}"
    echo "Local mountpoint: ${MOUNTPOINT}"
    echo "Hidden cache dir: ${CACHE_DIR}"
    ...
    if [ -n "$(find "${MOUNTPOINT}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null || true)" ]; then
    ...
    if mountpoint -q "${CACHE_DIR}" 2>/dev/null; then
        echo "Cache tmpfs already mounted on ${CACHE_DIR}, remounting with new size..."
        sudo mount -o remount,size="${CACHE_TMPFS_KB}k",uid="$(id -u)",gid="$(id -g)",mode=700 tmpfs "${CACHE_DIR}"
    else
        echo "Mounting cache tmpfs on ${CACHE_DIR} ..."
        sudo mount -t tmpfs -o "size=${CACHE_TMPFS_KB}k,uid=$(id -u),gid=$(id -g),mode=700" tmpfs "${CACHE_DIR}"
    fi
...
monitor_rclone_cli() {
    ...
    local cache_used cache_avail cache_total
    ...
    while ps -p "${RCLONE_PID}" >/dev/null 2>&1; do
        ...
        read -r cache_used cache_avail cache_total < <(
            df -B1 --output=used,avail,size "${CACHE_DIR}" 2>/dev/null | awk 'NR==2{print $1,$2,$3}'
        )
        ...
        echo
        echo "cache tmpfs : $(human_bytes "${cache_used:-0}") / $(human_bytes "${cache_total:-0}")"
        printf "cache fill  : "
        render_bar "${cache_used:-0}" "${cache_total:-1}" 36
        echo
        echo "vfs max use : $(human_bytes "${cache_used:-0}") / $(human_bytes "$(( CACHE_MAX_MIB * 1024 * 1024 ))")"
        printf "vfs target  : "
...
cleanup() {
    ...
    if [ -n "${CACHE_DIR:-}" ] && mountpoint -q "${CACHE_DIR}" 2>/dev/null; then
        echo "Unmounting RAM cache tmpfs..."
        sudo umount "${CACHE_DIR}" >/dev/null 2>&1 || sudo umount -l "${CACHE_DIR}" >/dev/null 2>&1 || true
    fi

    if [ "${CREATED_CACHE_DIR}" = "true" ] && [ -d "${CACHE_DIR}" ]; then
        rmdir "${CACHE_DIR}" >/dev/null 2>&1 || true
    fi

    if [ "${CREATED_MOUNTPOINT}" = "true" ] && [ -d "${MOUNTPOINT}" ]; then
