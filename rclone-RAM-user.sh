...
need_cmds() {
    for cmd in rclone awk numfmt ps id mountpoint mount grep sed tr printf sudo mkdir chmod chown ss tail nemo mktemp rmdir; do
...
launch_nemo() {
    ...
    echo "--- Press CTRL+C to close Nemo, unmount rclone, and unmount the RAM cache ---"
    echo "--- Local mountpoint: ${MOUNTPOINT} ---"
    echo "--- Hidden cache dir: ${CACHE_DIR} ---"
    echo "--- Check cache usage: du -sh ${CACHE_DIR} && df -h ${CACHE_DIR} ---"
